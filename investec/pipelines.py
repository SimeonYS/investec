...
class IinvestecPipeline:

    # Database setup
    conn = sqlite3.connect('investec.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `investec`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM investec WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO investec (date, title, link, content)"
